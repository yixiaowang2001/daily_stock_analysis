...
`/api/v1/agent-backtest` is a new multi-operator paper-trading experiment API for A-share watchlists. It creates isolated short-term, medium-term, and long-term profiles by default, and it can later add more active profiles such as a more aggressive short-term trader. It records when each profile joined, what it could see, what it decided, when the order became effective, how it filled, and the resulting daily NAV. It does not replace the historical-advice backtest above. The MVP enforces 6-digit A-share symbols, 100-share board lots, T+1 sell availability, buy-side cash checks, and isolated portfolio ledgers. The experiment decouples the watchlist from current holdings: each review cycle researches the run watchlist plus every symbol currently held by any active operator account, then gives every profile the same `symbol_facts` structure across market data, technicals, fundamentals, information, and sentiment. The context exposes only a de-identified `portfolio_held_symbols` universe, not other profiles' sizes, costs, or decisions. A symbol removed from the run watchlist but still held by the current profile is marked as that profile's `exit_only_symbols`, while symbols held only by other profiles and not in the watchlist are `research_only_symbols` for market awareness rather than buy candidates. The standalone Web entry is `/agent-backtest` ("操盘") for comparing active operators with daily post-16:00 NAV return curves, account-equity curves, current equity, strategy version, joined time, latest decision, and event stream. Register Codex Desktop automations with `python scripts/install_agent_backtest_codex_automations.py --run-id <id>` to schedule `0940看盘`, `1030看盘`, `1120看盘`, `1335看盘`, and `1440看盘` review cycles plus the 16:05 `收盘复盘` task. The close review records NAV, asks each profile for self-review, and can persist a forward-only policy version when there is a durable lesson. The detailed Chinese guide is [Agent 回测实验台](agent-backtest-workbench.md).
...
The WebUI and FastAPI API share the same service process. After startup, use the browser workspace for configuration management, manual analysis, task progress, historical reports, backtesting, portfolio management, and smart import. The standalone "操盘" page (`/agent-backtest`; `/trading` redirects there) shows the default short/medium/long paper-trading profiles plus any later-added isolated profiles; see [Agent 回测实验台](agent-backtest-workbench.md). Authentication, cloud-server access, and API usage details are covered below.
...
| `/api/v1/agent-backtest/runs/{run_id}` | GET / PATCH | Get run details; maintain the observed stock pool and daily decision limit |
| `/api/v1/agent-backtest/runs/{run_id}/profiles` | POST | Add an isolated operator profile with its own portfolio account and initial policy version |
| `/api/v1/agent-backtest/runs/{run_id}/profiles/{profile_key}` | DELETE | Mark an operator profile inactive while preserving its history and stopping future automation cycles |
| `/api/v1/agent-backtest/runs/{run_id}/profiles/{profile_key}/policies` | GET / POST | Browse policy-version history read-only, or append a new forward-only policy version |
